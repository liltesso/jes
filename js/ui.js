      ...
      <div class="px5" style="display:flex;justify-content:center;gap:16px;padding-bottom:24px;">
        ${featured.map((p, i) => `
          <div style="width:112px;height:172px;${i===1 ? "margin-top:16px;" : ""}border-radius:18px;overflow:hidden;position:relative;box-shadow:0 12px 32px rgba(161,59,224,0.3);">
            ${p.photo
              ? `<img src="${getPhoto(p.photo)}" style="width:100%;height:100%;object-fit:cover;" onerror="this.parentElement.style.background='${gradFor(p.id)}';this.remove();">`
...
function productCard(p) {
  ...
  const photoBlock = p.photo
    ? `<img src="${getPhoto(p.photo)}" style="width:100%;height:180px;object-fit:cover;border-radius:16px;box-shadow:0 8px 20px rgba(0,0,0,0.3);" onerror="this.outerHTML=${JSON.stringify(placeholder).replace(/"/g, "&quot;")};">`
    : placeholder;
  return `
