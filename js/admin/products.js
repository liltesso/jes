      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        <div class="card" style="padding:12px 16px;display:flex;flex-direction:column;gap:12px;">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <span style="font-size:14.5px;">Безкінечність (Infinity)</span>
            <input type="checkbox" ${p.stock_count >= 999999 || (!("stock_count" in p) && p.stock) ? "checked" : ""} onchange="state.adminEditing.stock_count = this.checked ? 999999 : 0; state.adminEditing.stock = this.checked; renderAdmin()" style="width:20px;height:20px;">
          </div>
          ${(!p.stock_count || p.stock_count < 999999) ? `
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <span style="font-size:14.5px;">Кількість (Шт.)</span>
            <input type="number" min="0" value="${p.stock_count !== undefined ? p.stock_count : (p.stock ? 1 : 0)}" oninput="state.adminEditing.stock_count = parseInt(this.value) || 0; state.adminEditing.stock = (parseInt(this.value) || 0) > 0" style="width:100px;font-size:14.5px;text-align:right;background:rgba(255,255,255,0.1);border:none;border-radius:6px;padding:4px 8px;color:#fff;">
          </div>` : ""}
        </div>
        <div class="card" style="padding:12px 16px;display:flex;justify-content:space-between;align-items:center;">
...
async function adminPhotoSelected(e) {
  ...
  if (statusEl) statusEl.textContent = "Завантаження на ImgBB...";

  try {
    // ЗАДАЧА 3: Vercel Serverless Fix - завантаження на ImgBB замість локального сервера
    const IMGBB_API_KEY = "8265870dfb4490cf65eab1dc4697d88f"; // ТРЕБА ЗАМІНИТИ НА РЕАЛЬНИЙ КЛЮЧ!
    
    if (IMGBB_API_KEY === "8265870dfb4490cf65eab1dc4697d88f") {
       alert("УВАГА: Вставте ваш API-ключ від ImgBB у файл public/js/admin/products.js (рядок 109)!");
    }
    ...
    if (!res.ok || !data.success) {
       throw new Error(data.error?.message || "Помилка завантаження ImgBB");
    }

    // Зберігаємо абсолютний URL картинки (https://i.ibb.co/...)
    state.adminEditing.photo = data.data.url;
    
    if (statusEl) statusEl.textContent = "Успішно завантажено ✓";
    renderAdmin();
