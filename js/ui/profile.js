  ...
  return `
    <div style="padding-top:8px;padding-bottom:16px;">
      ...
      <div class="px5" style="margin-bottom:20px;">
        ...
        <div class="card"><input id="topupAmountInput" type="number" inputmode="numeric" placeholder="${t("topup_ph")}" value="${state.topupAmount}" style="width:100%;padding:14px 16px;font-size:14.5px;" oninput="state.topupAmount=this.value;render()"></div>
      ...
      <div class="px5" style="margin-bottom:24px;">
        <div style="font-weight:700;font-size:15px;margin-bottom:10px;">Спосіб оплати / Zahlungsmethode</div>
        <div style="display:flex;flex-direction:column;gap:8px;">
          ${methods.map(m => `
            <div class="card" style="padding:14px 16px;display:flex;align-items:center;cursor:pointer;border:${state.topupMethod===m.id ? '2px solid var(--tg-theme-button-color, #3390ec)' : '2px solid transparent'}" onclick="state.topupMethod='${m.id}';render()">
              <div style="flex:1;font-size:14.5px;font-weight:600;">${m.label}</div>
              <div style="width:20px;height:20px;border-radius:50%;border:2px solid ${state.topupMethod===m.id ? 'var(--tg-theme-button-color, #3390ec)' : 'rgba(255,255,255,0.2)'};display:flex;align-items:center;justify-content:center;">
                ${state.topupMethod===m.id ? '<div style="width:10px;height:10px;border-radius:50%;background:var(--tg-theme-button-color, #3390ec);"></div>' : ''}
              </div>
            </div>
          `).join("")}
        </div>
