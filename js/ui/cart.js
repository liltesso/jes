  ...
  return `
    <div style="padding-top:8px;padding-bottom:16px;animation:fadeIn 0.3s ease;">
      ...
      <div class="card px5" style="margin:0 20px 16px;padding:20px;">
        <div style="font-weight:700;font-size:15px;margin-bottom:12px;">Промокод (якщо є)</div>
        <div style="display:flex; gap:8px;">
          <input id="promoInput" placeholder="Введіть код" value="${state.promoCode || ""}" oninput="state.promoCode=this.value" style="flex:1; border-radius:12px;background:rgba(0,0,0,0.4);border:1px solid rgba(255,255,255,0.1);padding:12px 16px;font-size:14.5px;">
          <button class="btn btn-outline" style="padding:0 16px;" onclick="applyPromo()">✅</button>
        </div>
        ${state.appliedPromo ? `<div style="color:#34d67f;font-size:13px;margin-top:10px;">✅ Знижка застосована: -${state.appliedPromo.discount}%</div>` : ``}
      </div>

      <!-- Totals -->
      <div class="card" style="margin:0 20px 24px;padding:16px 20px;">
        ${state.appliedPromo ? `
          <div style="display:flex;justify-content:space-between;font-size:14px;color:rgba(255,255,255,0.4);margin-bottom:6px;"><span>Сума</span><span>${total}€</span></div>
          <div style="display:flex;justify-content:space-between;font-size:14px;color:#34d67f;margin-bottom:10px;"><span>Знижка ${state.appliedPromo.discount}%</span><span>-${Math.round(total*state.appliedPromo.discount/100)}€</span></div>
        ` : ""}
        <div style="display:flex;align-items:center;justify-content:space-between;">
          <span style="font-size:17px;font-weight:600;">Разом до сплати</span>
      ...
      ${crossSells.length > 0 ? `
        <div style="margin-bottom:24px;">
          <div style="padding:0 20px; font-weight:800; font-size:16px; margin-bottom:12px;">З цим часто купують 🔥</div>
            ...
            ${crossSells.map(p => {
              ...
              return `
                <div class="card" style="min-width:140px; padding:12px; border-radius:12px; scroll-snap-align:start; display:flex; flex-direction:column; align-items:center; text-align:center;">
                  ...
                  <button class="btn btn-primary" style="padding:4px 12px; font-size:11px; width:100%;" onclick="addToCart(${p.id})">Додати</button>
                </div>
              `;
            }).join("")}
          </div>
      ...
      <div class="px5">
        <button class="btn btn-primary" style="width:100%; padding:14px; font-size:16px; font-weight:800;" onclick="state.checkoutStep='delivery';render()">Оформити замовлення 🚀</button>
      </div>
    </div>
  `;
}
...
function getStepper(stepIndex) {
  const steps = ['Корзина', 'Доставка', 'Оплата', 'Підтвердження'];
  return `
...
function renderStepDelivery() {
  return `
    <div style="padding-top:16px;padding-bottom:24px;animation:fadeIn 0.3s ease;">
      ...
      <div class="px5" style="margin-bottom:20px;">
        <h3 style="margin:0 0 16px 0; font-size:18px;">Спосіб отримання</h3>
        <div style="display:flex;gap:12px;margin-bottom:20px;">
          <div class="card" style="flex:1; padding:16px; text-align:center; cursor:pointer; border: 2px solid ${state.delivery==='pickup' ? '#c026d3' : 'transparent'};" onclick="state.delivery='pickup';render()">
            ...
            <div style="font-weight:700;font-size:14px;">Самовивіз</div>
          </div>
          <div class="card" style="flex:1; padding:16px; text-align:center; cursor:pointer; border: 2px solid ${state.delivery==='delivery' ? '#c026d3' : 'transparent'};" onclick="state.delivery='delivery';render()">
            <div style="font-size:24px;margin-bottom:8px;">🚚</div>
            <div style="font-weight:700;font-size:14px;">Пошта / Кур'єр</div>
          </div>
        </div>

        ${state.delivery === "pickup" ? `
          <div style="animation:fadeIn 0.3s ease;">
            <div style="font-weight:700;font-size:14px;margin-bottom:10px;">Оберіть місто для самовивозу:</div>
            <div style="display:flex;flex-direction:column;gap:8px;">
              ${CITIES.map(c => `<button class="btn ${state.city===c.name ? 'btn-primary' : 'btn-outline'}" onclick="state.city='${c.name}';render()">${c.name}</button>`).join("") || '<div style="color:rgba(255,255,255,0.3);font-size:13px;">Немає доступних міст</div>'}
            </div>
          </div>
        ` : ""}
        ...
        ${state.delivery === "delivery" ? `
          <div style="animation:fadeIn 0.3s ease;">
            <div style="font-weight:700;font-size:14px;margin-bottom:10px;">Адреса доставки:</div>
            <div class="card"><input id="addrInput" placeholder="Місто, Відділення НП або Адреса" value="${state.city || ''}" style="width:100%;padding:14px 16px;font-size:14.5px;" oninput="state.city=this.value"></div>
          </div>
        ` : ""}
      </div>
      ...
      <div class="px5" style="display:flex; gap:12px;">
        ...
        <button class="btn btn-primary" style="flex:2;" ${(!state.delivery || !state.city) ? "disabled style='opacity:0.4;'" : ""} onclick="state.checkoutStep='payment';render()">Далі</button>
      </div>
    </div>
  `;
}
  ...
  return `
    <div style="padding-top:16px;padding-bottom:24px;animation:fadeIn 0.3s ease;">
      ...
      <div class="px5" style="margin-bottom:24px;">
        <h3 style="margin:0 0 16px 0; font-size:18px;">Метод оплати</h3>
        <div style="display:flex;flex-direction:column;gap:12px;">
          <div class="card" style="padding:16px; display:flex; align-items:center; gap:12px; cursor:pointer; border: 2px solid ${state.payment==='mono' ? '#c026d3' : 'transparent'};" onclick="state.payment='mono';render()">
            ...
            <div>
              ...
              <div style="font-size:12px;color:rgba(255,255,255,0.4);">Переказ на картку (Україна)</div>
            </div>
          </div>
          ...
          <div class="card" style="padding:16px; display:flex; align-items:center; gap:12px; cursor:pointer; border: 2px solid ${state.payment==='sparkasse' ? '#c026d3' : 'transparent'};" onclick="state.payment='sparkasse';render()">
            ...
            <div>
              ...
              <div style="font-size:12px;color:rgba(255,255,255,0.4);">Банківський переказ IBAN (Європа)</div>
            </div>
          </div>
          ...
          <div class="card" style="padding:16px; display:flex; align-items:center; gap:12px; cursor:pointer; border: 2px solid ${state.payment==='cash' ? '#c026d3' : 'transparent'};" onclick="state.payment='cash';render()">
            <div style="font-size:24px;">💵</div>
            <div>
              <div style="font-weight:700;font-size:15px;">Готівка</div>
              <div style="font-size:12px;color:rgba(255,255,255,0.4);">Оплата при отриманні</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="px5" style="display:flex; gap:12px;">
        ...
        <button class="btn btn-primary" style="flex:2;" ${(!state.payment) ? "disabled style='opacity:0.4;'" : ""} onclick="state.checkoutStep='confirm';render()">Підтвердити</button>
      </div>
    </div>
  `;
}
...
function renderStepConfirm(items, total, discountedTotal) {
  return `
    <div style="padding-top:16px;padding-bottom:24px;animation:fadeIn 0.3s ease;">
      ...
      <div class="px5" style="margin-bottom:24px;">
        <h3 style="margin:0 0 16px 0; font-size:18px;">Перевірте замовлення</h3>
        
        <div class="card" style="padding:16px; margin-bottom:16px;">
          <div style="font-weight:700;font-size:14px;color:#c026d3;margin-bottom:8px;">Товари (${items.length})</div>
          ${items.map(i => `
        ...
        <div class="card" style="padding:16px; display:flex;flex-direction:column;gap:12px;">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <div>
              ...
              <div style="font-size:13px;color:rgba(255,255,255,0.8);">${state.delivery === 'pickup' ? 'Самовивіз' : 'Пошта'} — ${state.city}</div>
            </div>
            <span onclick="state.checkoutStep='delivery';render()" style="font-size:12px;color:rgba(255,255,255,0.4);text-decoration:underline;cursor:pointer;">Змінити</span>
          </div>
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;">
            ...
            <span onclick="state.checkoutStep='payment';render()" style="font-size:12px;color:rgba(255,255,255,0.4);text-decoration:underline;cursor:pointer;">Змінити</span>
          </div>
        </div>
      </div>
      ...
      <div class="px5" style="display:flex; gap:12px;">
        ...
        <button class="btn btn-primary" style="flex:2;" onclick="submitCheckout()">Створити замовлення 🚀</button>
      </div>
    </div>
  `;
}
