            ...
            ${crossSells.map(p => `
              <div class="card" style="min-width:140px;padding:12px;border-radius:12px;scroll-snap-align:start;display:flex;flex-direction:column;align-items:center;text-align:center;">
                <div style="height:70px;display:flex;align-items:center;justify-content:center;margin-bottom:8px;">
                  ${p.photo ? `<div style="position:relative;width:60px;height:60px;border-radius:8px;overflow:hidden;"><div class="skeleton-block" style="position:absolute;inset:0;z-index:1;"></div><img src="${getPhoto(p.photo)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:contain;z-index:2;opacity:0;transition:opacity 0.3s;" onload="this.style.opacity=1;this.previousElementSibling.style.display='none';"></div>` : `<div style="font-size:24px;">🧴</div>`}
                </div>
  ...
  return `
    <div style="padding-top:16px;padding-bottom:24px;animation:fadeIn 0.3s ease;">
      ...
      <div class="px5" style="display:flex;gap:12px;">
        ...
        <button class="btn btn-primary" style="flex:2;" ${(!state.delivery||!state.city)?"disabled style='opacity:0.4;'":""} onclick="state.checkoutStep='payment';render()">${CURRENT_LANG==='de'?'Weiter':'Далее'}</button>
      </div>
    </div>
  `;
}
        ...
        <div style="display:flex;flex-direction:column;gap:12px;">
          ...
          <div class="card" style="padding:16px;display:flex;align-items:center;gap:12px;cursor:pointer;border:2px solid ${state.payment==='mono'?'#c026d3':'transparent'};" onclick="state.payment='mono';render()">
            ...
            <div>
              ...
              <div style="font-size:12px;color:rgba(255,255,255,0.4);">${t("pay_mono")}</div>
            </div>
          </div>
          <div class="card" style="padding:16px;display:flex;align-items:center;gap:12px;cursor:pointer;border:2px solid ${state.payment==='sparkasse'?'#c026d3':'transparent'};" onclick="state.payment='sparkasse';render()">
            <div style="font-size:24px;">🏦</div>
            <div>
              <div style="font-weight:700;font-size:15px;">Sparkasse</div>
              <div style="font-size:12px;color:rgba(255,255,255,0.4);">${t("pay_sparkasse")}</div>
            </div>
          </div>
