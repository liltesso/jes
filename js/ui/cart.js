...
function renderCart() {
  ...
  return `
    <div style="padding-top:8px;padding-bottom:16px;animation:fadeIn 0.3s ease;">
            ...
            ${crossSells.map(p => `
              <div class="card" style="min-width:140px;padding:12px;border-radius:12px;scroll-snap-align:start;display:flex;flex-direction:column;align-items:center;text-align:center;">
                <div style="height:70px;display:flex;align-items:center;justify-content:center;margin-bottom:8px;">
                  ${p.photo ? `<img src="${getPhoto(p.photo)}" style="height:60px;object-fit:contain;">` : `<div style="font-size:24px;">🧴</div>`}
                </div>
      ...
      <div class="px5">
        <button class="btn btn-primary" style="width:100%;padding:14px;font-size:16px;font-weight:800;" onclick="state.checkoutStep='delivery';render()">${t("checkout_action")} 🚀</button>
      </div>
    </div>
  `;
}
  ...
  return `
    <div style="padding-top:16px;padding-bottom:24px;animation:fadeIn 0.3s ease;">
      ...
      <div class="px5" style="display:flex;gap:12px;">
        ...
        <button class="btn btn-primary" style="flex:2;" ${!state.payment?"disabled style='opacity:0.4;'":""} onclick="state.checkoutStep='confirm';render()">${t("confirm_order")}</button>
      </div>
    </div>
  `;
}
