            ...
            ${crossSells.map(p => `
              <div class="card" style="min-width:140px;padding:12px;border-radius:12px;scroll-snap-align:start;display:flex;flex-direction:column;align-items:center;text-align:center;">
                <div style="height:70px;display:flex;align-items:center;justify-content:center;margin-bottom:8px;">
                  ${p.photo ? `<div style="position:relative;width:60px;height:60px;border-radius:8px;overflow:hidden;"><div class="skeleton-block" style="position:absolute;inset:0;z-index:1;"></div><img src="${getPhoto(p.photo)}" loading="lazy" style="position:absolute;inset:0;width:100%;height:100%;object-fit:contain;z-index:2;opacity:0;transition:opacity 0.3s;" onerror="this.outerHTML='<div style=\'font-size:24px;\'>🧴</div>';" onload="this.style.opacity=1;this.previousElementSibling.style.display='none';"></div>` : `<div style="font-size:24px;">🧴</div>`}
                </div>
