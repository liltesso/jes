...
function productCard(p) {
  ...
  const photoBlock = p.photo
    ? `<div style="position:relative; width:100%; aspect-ratio:1/1; border-radius:12px; overflow:hidden;">
         ...
         <img data-src="${getPhoto(p.photo)}" style="position:absolute; inset:0; width:100%; height:100%; object-fit:contain; z-index:2; opacity:0; transition: opacity 0.3s;" onerror="this.outerHTML=${JSON.stringify(placeholder).replace(/"/g, "&quot;")};" onload="this.style.opacity=1; this.previousElementSibling.style.display='none';">
       </div>`
    : placeholder;
  return `
...
function renderProduct() {
  ...
  const imgBlock = p.photo ? `<img data-src="${getPhoto(p.photo)}" style="width:100%;aspect-ratio:1/1;object-fit:contain;background:rgba(255,255,255,0.02);" onerror="this.outerHTML=${JSON.stringify(placeholder).replace(/"/g, "&quot;")};">` : placeholder;
